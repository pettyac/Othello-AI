...
def simulate_game(tree,
    ...
    while True:
        ## GET ACTION ##
        next_action = get_move(node=current_node,
                               player=turn)
        ...
        current_state = apply_action(current_state,
                                        action=next_action,
                                        player=turn)
        current_node = SearchNode(current_state)
...
if __name__ == "__main__":
    ...
    board_state = [[' ', ' ', ' ', ' '],
                   ...
                   [' ', ' ', ' ', ' ']]
    board_size = 4
    tree = GameTree(SearchNode(board_state), 0, 0)
    ## Give these the get_move functions from whatever ais you want to test
    white_get_move = Brutus.get_move
    black_get_move = Randy.get_move
    simulate_game(tree, white_get_move, black_get_move)
